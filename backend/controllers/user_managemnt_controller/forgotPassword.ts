...
<style>
  ...
  .confirm-button { background-color: #1e3a8a; color:#ffffff; padding:14px 28px; border-radius:6px; text-decoration:none; font-weight:bold; display:inline-block; }
...
<div class="container" style="max-width: 600px; margin: 20px auto; padding: 30px; background: #ffffff; border-radius: 8px; color:#333333;">
  ...
  <div class="button-container" style="margin:20px 0;">
    <a href="${resetLink}" class="confirm-button" style="background-color: #1e3a8a; color:#ffffff; padding:14px 28px; border-radius:6px; text-decoration:none; font-weight:bold; display:inline-block;">Reset Password</a>
  </div>
...
const forgotPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resetLink = `${FRONTEND_URL}/resetPassword?token=${encodeURIComponent(resetToken)}`;
