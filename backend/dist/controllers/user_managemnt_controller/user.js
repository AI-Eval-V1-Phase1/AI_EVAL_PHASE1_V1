    ...
    try {
        ...
        if (String(role).toLowerCase().trim() === "admin") {
            const existingAdmin = await db
                ...
                .from(usersTable)
                .where(and(eq(usersTable.organization_name, organizationName), eq(usersTable.role, "admin")))
                .limit(1);
            if (existingAdmin.length > 0) {
                return res.status(400).json({
                    message: "This organization already has an admin. Only one admin per organization is allowed.",
                });
            }
        }
