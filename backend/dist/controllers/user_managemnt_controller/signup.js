...
const userSignup = async (req, res) => {
    ...
    const emailFromToken = req.email != null ? String(req.email).trim() : "";
    ...
    function userEmailTemplate(name, role, onboardingLink, organization) {
        return `<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
  ...
  <div class="footer" style="font-size:12px; color:#666666; margin-top:20px; text-align:center;">&copy; 2026 AI Eval. All rights reserved.</div>
    ...
    try {
        ...
        const orgUsersWithRole = await db
            ...
            .from(usersTable)
            .where(and(eq(usersTable.organization_name, orgName), isNotNull(usersTable.user_platform_role), ne(usersTable.user_platform_role, "")))
            .limit(1);
        let platformRoleToStore = null;
        if (isAiEvalOrg) {
        ...
        const isAdminUser = userOrgRole === "admin" || (isAiEvalOrg && userOrgRole === "system admin");
        ...
        const orgOnboardedUsers = await db
            ...
            .from(usersTable)
            .where(and(eq(usersTable.organization_name, dbUser.organization_name), eq(usersTable.user_onboarding_completed, "true")))
            .limit(1);
        ...
        if (isAdminUser) {
            ...
            if (!isOrgOnboardingCompleted) {
                ...
                await transporter.sendMail({
                    ...
                    html: userEmailTemplate(dbUser.user_name ?? "User", capitalizeFirstLetter(String(dbUser.role ?? "")), onboardingLink, capitalizeFirstLetter(String(orgDisplayName ?? ""))),
                });
                ...
                await db
                    ...
                    .where(eq(usersTable.id, dbUser.id));
                console.log("Org already onboarded; admin signup without onboarding email:", email);
            }
        }
        else {
            ...
            await db
                ...
                .where(eq(usersTable.id, dbUser.id));
            console.log("Non-admin signup; no onboarding email (onboarding skipped):", email);
        }
        res.status(201).json({
