...
const submitVendorCotsAssessment = async (req, res) => {
    try {
        ...
        const orgIdFromUser = user.organization_name;
        ...
        const body = req.body ?? {};
        const get = (key) => body[key] ?? body[key.replace(/_([a-z])/g, (_, c) => c.toUpperCase())];
        const parseJson = (v) => {
            if (v == null)
                return null;
